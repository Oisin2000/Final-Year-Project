...
function SearchScreen(props) {
          ...
          Alert.prompt(
            ...
            (myrating) => {
              if (myrating !== null) {
                ...
                fetch(
                  "https://thawing-shore-72198.herokuapp.com/save-movie-data",
                  {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
            ...
            (myrating) => {
              if (myrating !== null) {
                ...
                const data = {
                  ...
                  myrating: myrating || "No Rating",
                };

                // Make a request to add the movie to mymovies
                fetch(
                  "https://thawing-shore-72198.herokuapp.com/add-mymovies-from-recommended",
                  {
                    method: "POST",
                    body: JSON.stringify(data),
                    headers: {
  ...
  const handleAddWatch = () => {
    ...
    // Make a request to add the movie to mymovies
    fetch(
      "https://thawing-shore-72198.herokuapp.com/add-watchlist-from-recommended",
      {
        method: "POST",
        body: JSON.stringify(data),
        headers: {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.text}>
        Find your next favorite movie with our search feature!
      </Text>
      ...
      >
        ...
        <ScrollView style={styles.results}>
          ...
          <Text style={styles.modalTitle}>
            {searchstate.selectedfromSearch.Title}
          </Text>
          <Text style={styles.modaltext}>
            Rating: {searchstate.selectedfromSearch.imdbRating}
          </Text>
          <Text style={styles.modaltext}>
            Cast: {searchstate.selectedfromSearch.Actors}
          </Text>
          <Text style={styles.modaltext}>
            Director: {searchstate.selectedfromSearch.Director}
          </Text>
          <Text style={styles.modaltext}>
            Genre: {searchstate.selectedfromSearch.Genre}
          </Text>
          <Text style={styles.modaltext}>
            Plot: {searchstate.selectedfromSearch.Plot}
          </Text>
          <Text style={styles.modaltext}>
            Awards: {searchstate.selectedfromSearch.Awards}
          </Text>
      ...
      <View style={styles.buttonContainer}>
        <AppButton
          title={"See some of our favourites !"}
          color={colours.third}
          onPress={() => setFavouritesModal(true)}
        />
      </View>
      <Modal visible={favouriteModal} animationType="slide">
        <View style={styles.favouritecontainer}>
          <Text style={styles.modalsubTitle}>
            Some of our favourites from all genres !
          </Text>
          ...
          <View style={styles.buttonContainer}>
            <AppButton
              color={colours.third}
              title="Close"
              onPress={() => setFavouritesModal(false)}
            />
          </View>
        </View>
        <Modal visible={detailsModalVisible} animationType="slide">
          <ScrollView style={styles.results}>
            ...
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modaltext}>
                Box Office: {selectedMovie.box_office}
              </Text>
              <Text style={styles.modaltext}>
                Run Time: {selectedMovie.runtime}
              </Text>
              <Text style={styles.modaltext}>
                Released: {selectedMovie.released}
              </Text>
